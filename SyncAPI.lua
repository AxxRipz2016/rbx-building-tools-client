...
local function shouldRunActionsLocally()
	return Tool:GetAttribute 'BTClientOnly' or (RunService:IsClient() and RunService:IsStudio())
end
...
Actions = {
	...
	['Export'] = function (Parts)
		...
		if RunService:IsClient() and not shouldRunActionsLocally() then
...
function CanModifyItems(Items)
	...
	for _, Item in pairs(Items) do
		...
		-- Catch locked parts
		if Options.DisallowLocked and (Item:IsA 'BasePart') and Item.Locked and not Tool:GetAttribute 'BTClientOnly' then
			return false
		end
